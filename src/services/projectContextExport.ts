...
import { FORMATION_RATING_V3_TIER_THRESHOLDS } from './formationRatingTierV3';
...
function buildFormationRules() {
  return {
    ...
    semanticRelationships: {
      identity: ['providerDragonId', 'canonicalSemanticTag', 'beneficiaryDragonId'],
      classes: {
        'conditional-payoff': { baseValue: 10, activeSynergyCap: 30 },
        'output-amplification': { baseValue: 6, activeSynergyCap: 30 },
        'stat-support': { baseValue: 5, activeSynergyCap: 15 },
      },
      providerRedundancy: [1, 0.5, 0],
      controlAliasesCanonicalizeTo: 'status:control',
      lockedBlockedAndUnresolvedAreActive: false,
      recipientTargeting: {
        capabilityPriorityOne: {
          sharedSelectionGroup: true,
          uniquePriorityResolves: true,
          multiplePriorityUnresolved: true,
          uniqueFallbackResolves: true,
          multipleFallbackUnresolved: true,
          unresolvedRecipientCredit: 0,
          capabilitySource: 'active output signals at current progression and self position',
        },
      },
    },
    publicRating: {
      contract: 'formation-rating-v3',
      total: 100,
      categories: { 'Active Synergy': 80, 'Placement Effectiveness': 20 },
      participationBonus: { threeDragons: 5, twoDragons: 2, otherwise: 0 },
      tierThresholds: { ...FORMATION_RATING_V3_TIER_THRESHOLDS, Weak: 0 },
      reliability: {
        guaranteedAndProvenRetainFullValue: true,
        supportedChanceMayReduceContribution: true,
        unquantifiedPotentialScored: false,
        cumulativeRequiresExactOpportunitiesAndConfirmedIndependence: true,
      },
      incompleteIsValidationGate: true,
      analysisConfidenceScored: false,
      kitCoverageScored: false,
    },
    placementComparison: {
      permutationCount: 6,
      ...
      meaningfulLossFormula: 'round(20 * adjustedCurrentValue / adjustedBestValue)',
