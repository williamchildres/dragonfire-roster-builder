      ...
      schedule({
        ...
        effects: [
          ...
          fixedEffect({
            ...
            activationRoll: roll({ scope: 'effect', chanceFixed: 20, description: 'One 20% chance to grant a Spreading Blaze stack.' }),
            targetPriority: 'any-eligible',
            stack: stack({
  ...
  const habits = [
    ability({
      ...
      schedules: [
        ...
        schedule({ id: 'trial-by-flame-below-50', timing: 'start-of-each-round', conditions: [below50], effects: [fixedEffect({ id: 'trial-below-50-resistance', type: 'Resistance', target: 'All allies below 50% Troop Capacity', targetScope: 'any-lane', magnitude: null, unit: 'percent', rankedValues: rankedPercents([10, 12, 14, 17, 20]), duration: 'Until end of current round', conditions: [below50], targetPriority: 'all-allies-matching-threshold', casterEligibility: 'excluded', includesCaster: false, notes: ['Resistance reduces Damage Received.'] })] }),
      ...
      powerByHabitLevel: [],
      tags: ['FIRE_DAMAGE_RECEIVED_DOWN', 'RESISTANCE', 'DAMAGE_RECEIVED_DOWN'],
...
const targetSelection = (details: Partial<TargetSelectionDetails>): TargetSelectionDetails => ({
  ...
  qualifyingOutputAffectsEligibility: details.qualifyingOutputAffectsEligibility ?? false,
...
const createDaemoros = (): Dragon => {
  const fearEffects = (id: string, preference: TargetPriority) => [
    fixedEffect({ id: `${id}-intelligence`, type: 'Intelligence Down', target: `1 Enemy, prioritizing ${preference === 'prefer-right-flank' ? 'Right Flank' : 'Left Flank'}`, targetScope: 'any-lane', magnitude: 25, unit: 'percent', durationRounds: 2, scaling: ['enhanced by Daemoros Strength'], targetCount: 1, targetPriority: preference, targetSelection: targetSelection({ preference: preference === 'prefer-right-flank' ? 'Right Flank' : 'Left Flank', fallback: 'another eligible enemy', sharedSelectionGroupId: `${id}-target`, distinctness: 'same-target-required' }) }),
    fixedEffect({ id: `${id}-instinct`, type: 'Instinct Down', target: `Same selected enemy`, targetScope: 'any-lane', magnitude: 25, unit: 'percent', durationRounds: 2, scaling: ['enhanced by Daemoros Strength'], targetCount: 1, targetPriority: preference, targetSelection: targetSelection({ preference: preference === 'prefer-right-flank' ? 'Right Flank' : 'Left Flank', fallback: 'another eligible enemy', references: [{ id: `${id}-same-target`, kind: 'same-target-as-effect', referencedEffectId: `${id}-intelligence`, description: 'Uses the enemy selected for the Intelligence reduction.' }], sharedSelectionGroupId: `${id}-target`, distinctness: 'same-target-required' }) }),
    fixedEffect({ id: `${id}-panic`, type: 'Panic', target: 'Same selected enemy', targetScope: 'any-lane', magnitude: 20, unit: 'rate', durationRounds: 2, targetCount: 1, targetPriority: preference, targetSelection: targetSelection({ preference: preference === 'prefer-right-flank' ? 'Right Flank' : 'Left Flank', fallback: 'another eligible enemy', references: [{ id: `${id}-same-target`, kind: 'same-target-as-effect', referencedEffectId: `${id}-intelligence`, description: 'Uses the enemy selected for the stat reductions.' }], sharedSelectionGroupId: `${id}-target`, distinctness: 'same-target-required' }), notes: ['Panic is periodic Tactical Damage each round.'] }),
  ];
...
const createShadowsong = (): Dragon => {
  ...
  const blazingConductorEffects = [
    fixedEffect({ id: 'blazing-conductor-first-fire', type: 'Fire Damage', target: 'First added enemy in any lane', targetScope: 'any-lane', magnitude: null, unit: 'rate', rankedValues: rankedPercents([60, 78, 96, 120, 150]), scaling: ['attacker Intelligence'], targetCount: 1, targetSelection: targetSelection({ sharedSelectionGroupId: 'blazing-conductor-first-target', distinctness: 'no-distinctness-requirement' }), notes: ['Mitigated by target Initiative.'] }),
    burnStatusEffect('blazing-conductor-first-burn', 'First added enemy', [40, 52, 64, 80, 100], 'blazing-conductor-first-fire'),
    fixedEffect({ id: 'blazing-conductor-second-fire', type: 'Fire Damage', target: 'Second added enemy in any lane, different from the first added target', targetScope: 'any-lane', magnitude: null, unit: 'rate', rankedValues: rankedPercents([30, 39, 48, 60, 75]), scaling: ['attacker Intelligence'], targetCount: 1, targetSelection: targetSelection({ references: [{ id: 'blazing-conductor-distinct-first-target', kind: 'distinct-from-effect-target', referencedEffectId: 'blazing-conductor-first-fire', description: 'Second added target must differ from the first added target.' }], sharedSelectionGroupId: 'blazing-conductor-second-target', distinctness: 'must-be-distinct' }), notes: ['Mitigated by target Initiative.', 'No requirement is invented that either added target differs from the two base adjacency targets.'] }),
