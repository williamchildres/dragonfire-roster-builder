# Full-roster regression audit — 0.9.0
...
- Source database: 0.9.0; data schema 13; local roster schema 4.
...
- Recorded audit runtime: 6936 ms.
...
The 0.6.8 deterministic hash was `2a4561cdb2aa6d0b9483005f44cc3ee3747d21fb6c4ecb1fe0cc375c1dafbf64`; the 0.9.0 hash is `2a4561cdb2aa6d0b9483005f44cc3ee3747d21fb6c4ecb1fe0cc375c1dafbf64`. All 26,970 numeric scores and component totals unchanged: Yes.

## Browser QA

Status: PASS_WITH_PENDING_REAL_200_PERCENT_ZOOM. Audited URLs: http://127.0.0.1:4190/ (initial local preview) and http://127.0.0.1:4191/ (amendment local preview). The 390×844 amendment flow preserved Star Rank sort, cleared only a nonmatching search to reveal Syrax, selected and opened Syrax, returned in list mode after Back → Overview → Roster, then selected and opened Vhagar exactly once. Document overflow: 0; clipped interactive controls: 0; console errors: 0; console warnings: 0.

Autosave wording used only local/fake states: browser-only while signed out; active account synchronization while fake signed-in and synced; and status-deferred wording while fake paused, offline, error, or conflict. No production account was automated.
