...
- Source database: 0.6.8; data schema 13; local roster schema 4.
...
Finding totals by severity: informational 2, low 2, medium 2. By category: browser-only observation 1, presentation defect 2, relationship-aggregation defect 1, unresolved by design 1, unsupported by current simple evaluator 1.

### FRR-F003 — relationship-aggregation defect (medium)

- Affected area: Aggregated relationship ability references
- Ability/profile signal: crimson-hunters-cunning, malachite-sentinels-presence, shadowrend-warriors-zeal, syrax-sentinels-wit, vermax-warriors-zeal
- Current behavior: 8487 ordered formations contain an active semantic relationship whose merged abilityIds include at least one currently locked or position-inactive alternative.
- Expected behavior: An active relationship may aggregate equivalent active paths, but its displayed ability references should not include inactive or position-blocked alternatives.
- Reproduction: syrax/vhagar/solstryker -> amplifier-output:syrax:stat:instinct:solstryker includes syrax-sentinels-wit; syrax/vhagar/crimson -> amplifier-output:crimson:damage:physical:vhagar includes crimson-hunters-cunning; syrax/vhagar/kalspire -> amplifier-output:syrax:stat:instinct:kalspire includes syrax-sentinels-wit
- Files: `src/synergy/evaluateFormation.ts`, `src/synergy/formationPresentation.ts`, `src/app/formationCardPresentation.ts`
- Focused automated reproduction: Yes
- Controller mechanic confirmation needed: No
- Recommended next action: Create a narrow evaluator-aggregation follow-up that filters merged ability IDs by the winning eligibility rank, with focused presentation regression coverage.
...
### FRR-F004 — presentation defect (medium)

- Affected area: Dragon Details — At a glance progression presentation
- Ability/profile signal: dawnseeker-first-light-first-strike; vesper-midnight-onslaught-confusion
- Current behavior: At 9 Stars, the ability cards correctly show First Light and Midnight Onslaught as Locked preview, but At a glance still lists Dawnseeker First-Strike and Vesper Confusion/Control as unqualified Provides signals.
- Expected behavior: Future signals may remain visible as previews, but the At a glance summary must either exclude them or clearly mark their inactive 10-Star unlock state.
- Reproduction: Open Dawnseeker Details at 9 Stars and compare the locked First Light card with the Provides list; repeat with Vesper Midnight Onslaught at 9 Stars.
- Files: `src/app/dragonDetailPresentation.ts`, `src/app/DragonDetailModal.tsx`
- Focused automated reproduction: Yes
- Controller mechanic confirmation needed: No
- Recommended next action: Create a narrow Details-presentation follow-up that makes the At a glance summary progression-aware without changing profiles or evaluator behavior.
- Audit disposition: Not fixed in this audit PR.

### FRR-F005 — browser-only observation (low)

- Affected area: Details long-label wrapping
- Ability/profile signal: Shadowrend and Thunderstrike Details headings; Thunderstrike technical tag label
- Current behavior: No page or dialog overflows its viewport and no chips are clipped, but 10 desktop descendant elements and one sampled mobile descendant report horizontal scrollWidth beyond clientWidth. Thunderstrike heading descendants exceed their desktop containers by 24–25 px; its mobile technical tag text exceeds by 24 px.
- Expected behavior: Long names and technical labels should wrap within their own element boxes so the horizontally overflowing element count is zero.
- Reproduction: At 1440×1000 open Shadowrend and Thunderstrike Details at 10 Stars and compare descendant scrollWidth/clientWidth; at 390×844 inspect Thunderstrike technical tags.
- Files: `src/styles/global.css`, `src/app/DragonDetailModal.tsx`
- Focused automated reproduction: Yes
- Controller mechanic confirmation needed: No
- Recommended next action: Add a narrow CSS wrapping fix and viewport regression test in a separate UI PR.
- Audit disposition: Not fixed in this audit PR.

### FRR-F006 — presentation defect (low)

- Affected area: About coverage copy
- Ability/profile signal: 31/31 roster coverage statement
- Current behavior: Overview, README, and selectors state complete 31/31 coverage, while About contains no current 31/31 coverage statement.
- Expected behavior: Overview, README, About, and selectors should consistently communicate current 31/31 coverage.
- Reproduction: Open Overview, About, and the Add Dragon selector, then compare their coverage copy with README.md.
- Files: `src/app/App.tsx`, `README.md`
- Focused automated reproduction: Yes
- Controller mechanic confirmation needed: No
- Recommended next action: Add one concise 31/31 coverage sentence to About in a separate presentation-only PR.
- Audit disposition: Not fixed in this audit PR.

## Browser QA

Status: PASS with classified observations. Audited URL: https://dragonfirelab.com. Selector traversal: 31; Details dialogs opened: 70; overflow: 11; clipped chips: 0; console errors: 0; console warnings: 0; share link: PASS — #formation=left-flank:caraxes,vanguard:syrax,right-flank:feskar generated and restored after reload..
...
Focused `it.todo` markers (6): `FRR-F001`, `FRR-F002`, `FRR-F003`, `FRR-F004`, `FRR-F005`, `FRR-F006`.
