...
HiGHS reports objectives as floating-point numbers even for integer models. In 0.19.1, an integral real-world 33-dragon solve exposed a contaminated `0.8403320312499968` objective during Backup stable chunk 0–48. The assignment itself was integral and feasible, so each scalar, histogram, and stable phase now validates every Boolean/integer variable against the strict `1e-7` integrality tolerance, rounds only those validated individual variables, and reconstructs the phase value with safe-integer arithmetic from the known integer coefficients. The reconstructed integer—not the floating aggregate—is fixed. Materially fractional variables, unsafe coefficients/results, and exact replay mismatches remain hard errors. Post-solve validation recomputes every fixed phase from the final assignment.
