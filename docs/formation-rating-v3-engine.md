...
Persistence, cloud synchronization, routes, sharing, and Estimated Power remain separate from the rating cutover. The historical v2 audit consumes `historicalFormationRatingV2Profiles`, which freezes its original Blazing Fury input while current production uses the corrected profile. Historical expected hashes are not rewritten.
