...
- known probability with unresolved repetition falls back to the documented probability for one confirmed opportunity;
...
interface AbilityReliabilityComponent {
  ...
  independence: 'confirmed' | 'reasonable-model-assumption' | 'unknown' | 'contradicted' | 'not-applicable';
...
| --- | --- | --- | --- | --- | --- | --- |
| A — direct `p` | Very high | Ignores supported repetition and target structure | Low | Probability only | Stable but systematically conservative | Safe fallback, not complete model |
...
| E — evidence-aware hybrid | High | Uses cumulative only where supported and direct `p` otherwise | Lowest supported precision | Explicit component evidence | Predictable and auditable | Recommended |
...
3. One supported opportunity: reliability `p`.
4. Repeated opportunities with supported actual `n` and confirmed independence: `1 - (1-p)^n`.
5. Known `p`, unresolved repetition or independence: use direct `p` as the conservative one-confirmed-opportunity fallback.
...
| --- | --- |
| Known `p`, unknown opportunity count | Direct `p`; label “Probability known; activation frequency unresolved.” |
| Known schedule, battle may end early | Direct `p` unless an observation window explicitly defines actual `n`. |
| Known opportunities, unknown independence | Direct `p`; do not use cumulative probability. |
...
| Ability activation frequency unknown | Direct `p` for one confirmed opportunity. |
...
This fallback avoids treating unresolved chance as guaranteed and avoids inventing a fixed “unknown” percentage.
...
| `velar-whirlwind-advantage-damage` | Producing 6-value amplification | Chance | `p=20%`; rounds 2, 4, 6, 8; one shared roll affects two other Allies; independence across rounds unknown |
| `velar-gales-of-power-first-strike` | Producing 10-value conditional payoff | Chance | Habit 1–5 `p=12%, 14.4%, 16.8%, 20.4%, 24%`; four scheduled rounds; three targets; separate per target/effect; independence unknown |
| `velar-gales-of-power-slow` | Producing 10-value conditional payoff | Chance | Same progression and schedule as First-Strike; separate effect and enemy-target checks |
...
- direct `p` for either Gales effect is 24%;
...
- Whirlwind Advantage direct `p` is 20%; a four-round independence assumption gives `1 - 0.8^4 = 59.04%`.
...
| Formation | Relationship | Current marginal | Direct-`p` / hybrid | Cumulative assumption |
| --- | --- | ---: | ---: | ---: |
...
| A — direct `p` | 65 | 76 | 47 | Sheepstealer becomes first |
...
| E — recommended evidence-aware fallback | 65 | 76 | 47 | Same as direct `p` until independence evidence exists |

The diagnostic supports the limitation: chance weighting can change a specific comparison. It does not support a categorical statement that Velar is overrated. Deterministic Velar relationships remain substantial, different trios expose different chance paths, and v3 placement must be recalculated.
...
| Repeated scheduled chance | Crimson Bloodscale Terror Stun; five odd-round checks, with a Habit-dependent Round 1 replacement | Direct `p` fallback until actual `n` and temporal independence are supported |
...
| Unresolved opportunity count | Tairax Gift of Fire | `p` by Habit Level is known; checks depend on battle length and Burned-Enemy count |
...
4. Compute `reliability` without rounding.
5. Compute `adjustedBaseValue = baseValue × reliability`.
...
> Base relationship value: 10; activation reliability: 40.96%; adjusted contribution: 4.10.
> Evidence: two confirmed independent 20% opportunities

When evidence is incomplete:

- “Reliability based on one confirmed opportunity.”
