...
The production audit in `src/audit/formationReliabilityRegistryAudit.ts` reconciles registry facts with the historical research inventory. Intentional structural differences—including split independent rolls, explicit Habit sources, round-specific overrides, binding-selected variants, and resolved mixed paths—are reported deterministically. Production does not import or parse the research report at runtime.
...
Habit-dependent probability separately carries `habitAbilityId`. It may match the component owner or identify an augmenting Habit. Validation requires a canonical Habit and complete levels 1–5 with no extra keys.
...
Paths are alternatives; events inside one path are jointly required; components inside one event share activation identity. Variant components require a branch ID on every binding reference.

Structured path applicability identifies either a probability context or a stable relationship use. This resolves the three formerly mixed signals without changing their current `SynergySignal` rows:

- Shadowsong: deterministic Breath of Fire damage versus Panic-enhanced Scorched Earth application;
- Shimmer: chance Command buffs versus deterministic Tactical Damage and Recovery;
- Zivern: deterministic Battle Mastery versus chance-based Fearsome Reach.

Vaeldra’s composite follow-on uses two explicit joint paths: Lure Taunt plus the deterministic follow-on, or Siren’s Call Taunt plus that follow-on. No branch depends on display parsing or dragon-specific scorer logic.

## Validation and CI

`contract` mode validates partial metadata. `full-migration` requires explicit binding classes, complete scoring coverage, canonical component and Habit sources, valid variants, referenced components, and zero unresolved mixed bindings.
...
The historical research hash remains independent. A future Formation Rating v3 engine may consume these registry surfaces and binding paths; no v2 evaluator imports them today.
