...
All strategies are exact, joint, zero-gap MILP allocations rather than greedy lists. Star Rank and Dragon Level control unlocks, ratings, and Estimated Power. Habit Levels remain stored and are preserved in Formation Builder handoff but are unweighted. Formation Rating v2 is unchanged, no dragon repeats, and no combat simulation occurs. The last completed Optimizer result survives section navigation within the current app session; it remains visible as stale after relevant roster or strategy changes, and is not stored across a browser reload. See [`docs/ROSTER_OPTIMIZER.md`](docs/ROSTER_OPTIMIZER.md) and the deterministic audits under [`docs/audits/`](docs/audits/).
